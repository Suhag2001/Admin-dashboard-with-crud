...
function BusCategoryManagment() {
  ...
  useEffect(() => {
    setFilteredEntries(
      data.filter((entry) =>
        entry.businesscategory.toLowerCase().includes(searchText.toLowerCase())
      )
    );
  }, [searchText, data]);

  const handleSearch = () => {
    const userInfo = "http://localhost:8090/MynetworkManager/businesscategory";
    fetch(userInfo)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8090/MynetworkManager/businesscategory/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const getCategoryData = () => {
    fetch("http://localhost:8090/MynetworkManager/businesscategory")
      .then((res) => res.json())
      .then((data) => {
    ...
    <>
      <div className="sidebar-distance">
        <nav className="navbar-page">
          <h4>Business Category Managment</h4>
        </nav>
...
export default BusCategoryManagment;
